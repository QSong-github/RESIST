# =============================================================================
# RESIST | Module A - Characterization
# lib/netVisual_bubble.R
# -----------------------------------------------------------------------------
# Vendored, lightly patched copy of CellChat::netVisual_bubble that drops
# interactions with NA communication probability before plotting. Kept inside
# the repository so that A2 does not depend on a locally modified CellChat
# installation.
#
# Origin : scripts/netVisual_bubble.R
# =============================================================================
...
netVisual_bubble <- function(object, sources.use = NULL, targets.use = NULL, signaling = NULL, pairLR.use = NULL, sort.by.source = FALSE, sort.by.target = FALSE, sort.by.source.priority = TRUE, color.heatmap = c("Spectral","viridis"), n.colors = 10, direction = -1, thresh = 0.05,
                             ...
                             return.data = FALSE,
                             filter.na.source.target = TRUE){  # NEW: option to drop NA source.target rows
  ...
  if (is.null(comparison)) {
    ...
    # NEW: filter out rows where source.target became NA (e.g., groups not in idents levels)
    if (filter.na.source.target) {
      df <- df[!is.na(df$source.target), , drop = FALSE]
    }
  } else {
    ...
    # NEW: filter out rows where source.target became NA (e.g., pairs not in cells.order)
    if (filter.na.source.target) {
      df <- df[!is.na(df$source.target), , drop = FALSE]
    }
  }
