...
A2 reads `paths.data`, uses the inherited human CellChat database and SCT assay,
...
function from v2-2. Loop placement and its path were repaired, but CellChat output
and comparison direction still require HPC review.
...
it as a general spatial preprocessing pipeline. The previously undefined plotting
palette variable has been replaced with the shared palette.
